...
provider = TracerProvider(
    resource=Resource.create({
        "service.name": "licence-ocr"
    })
)

trace.set_tracer_provider(provider)

# Configure OTLP exporter (HTTP → Tempo)
...
try:
    ...
    logging.warning("[WARN] prometheus_client not installed. Install with `pip install prometheus_client` to enable metrics.")
...
# --- Prometheus metric definitions (if available) ---
if Counter and Histogram and Gauge:
    REQUEST_COUNT = Counter(
        "license_api_requests_total",
        "Total HTTP requests received by the license API",
        ["method", "endpoint", "http_status"]
    )
    REQUEST_LATENCY = Histogram(
        "license_api_request_duration_seconds",
        "Request latency in seconds",
        ["endpoint"]
    )
    EXTRACT_SUCCESS = Counter(
        "license_api_extract_success_total",
        "Number of successful extraction operations"
    )
    EXTRACT_FAILURE = Counter(
        "license_api_extract_failure_total",
        "Number of failed extraction operations"
    )
    INFLIGHT_REQUESTS = Gauge(
        "license_api_inflight_requests",
        "Number of in-flight requests"
    )
    TOKENS_TOTAL = Counter(
        "license_api_tokens_total",
        "Total tokens used by the OCR API",
        ["kind"]  # "prompt", "completion", "total"
    )
    REQUEST_DURATION_MS = Histogram(
        "license_api_request_duration_ms",
        "Request duration in milliseconds"
    )
else:
    REQUEST_COUNT = REQUEST_LATENCY = EXTRACT_SUCCESS = EXTRACT_FAILURE = INFLIGHT_REQUESTS = None
    TOKENS_TOTAL = REQUEST_DURATION_MS = None
...
def start_metrics_server():
    try:
        # start a simple HTTP server exposing /metrics on port 8001 bound to all interfaces
        start_http_server(port=8001, addr="0.0.0.0")
        print("Prometheus metrics server started on 0.0.0.0:8001")
    except Exception as e:
        print(f"[WARN] Could not start Prometheus metrics server: {e}")


# --- Simple middleware to instrument requests (if metrics available) ---
@app.middleware("http")
...
# Expose Prometheus metrics via FastAPI endpoint (in addition to start_http_server)
@app.get("/metrics")
async def metrics_endpoint():
    if generate_latest:
        try:
            ...
            logging.error(f"[METRICS] Failed to generate metrics: {e}")
...
if '__main__' == __name__:
    ...
    # Start a separate simple metrics HTTP server on 0.0.0.0:8001 if prometheus_client is available.
    # This exposes metrics for Prometheus scrapes at host:8001 by default (matches typical Prometheus target).
    try:
        if start_http_server:
            logging.info("Starting prometheus_client HTTP server on 0.0.0.0:8001 for /metrics")
            # start_http_server will start a simple HTTP server that serves /metrics on given port.
            start_http_server(addr="0.0.0.0", port=8001)
    except Exception as e:
        logging.warning(f"[WARN] Could not start prometheus metrics HTTP server on port 8001: {e}")
